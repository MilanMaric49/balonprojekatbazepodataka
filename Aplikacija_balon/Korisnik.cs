...
namespace Aplikacija_balon
{
    public partial class Korisnik : Form
    {
        ...
        {
            SqlDataAdapter adapter = new SqlDataAdapter("select * from korisnik where id = " + Program.id, Konekcija.Connect());
        ...
        {
            SqlDataAdapter adapter = new SqlDataAdapter("select rezervacija.id, korisnik.email, rezervacija.korisnik_id, objekat.naziv, rezervacija.objekat_id, datum, pocetak, kraj from rezervacija join korisnik on korisnik.id = rezervacija.korisnik_id join objekat on objekat.id = rezervacija.objekat_id where rezervacija.korisnik_id = " + Program.id, Konekcija.Connect());
        ...
        private void btn_dodaj_Click(object sender, EventArgs e)
        {
            if (rezervacije.Rows.Count == 0)
            {
                try
                {
                    ...
                    komanda.Parameters.Add(new SqlParameter("@korisnik_id", SqlDbType.Int, 4, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Current, Program.id));
            ...
            {
                MessageBox.Show("Vec ste rezervisali termin!");
            }
        }
